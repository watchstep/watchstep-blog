...
**Image**는 Docker Container를 만드는 데 사용되는 Read-Only template/blueprint이다. (Read-Only Not Write이므로, 변경 불가능함) 애플리케이션을 실행하는 데 필요한 모든 파일을 포함한 Docker file를 빌드해 Image를 생성한다. Image는 런타임일 때, Container가 된다. 즉, Container는 Docker Engine에서 실행한 Image이다.
여기서 Docker file이란, Image를 생성하고, 실행하는 데 사용되는 모든 명령을 포함한 텍스트 파일이다.
...
Image 붕어빵 틀로 Container 붕어빵을 만들면, 변경 불가능한 image layer 위에 Read-Write가 가능한 container layer가 추가되어 변경 가능해진다.
